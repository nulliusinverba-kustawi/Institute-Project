...
import RacialProfilingClient from './RacialProfilingClient'
import ExpandableText from '@/components/shared/ExpandableText'
import { buildMetadata } from '@/lib/metadata'

export async function generateMetadata(): Promise<Metadata> {
  return buildMetadata({ title: 'Racial Profiling — Research' })
}

export default async function RacialProfilingPage() {
  const [supabase, { data: sections }] = await Promise.all([
    createClient(),
    getPageContent('research'),
  ])
  const subtitle = sections?.find((s) => s.section === 'racial_profiling_description')?.content ?? ''

  const { data } = await supabase
    .from('research_posts')
    .select('id, title, excerpt, cover_path, external_url, author, item_type, created_at, image_fit')
    .eq('published', true)
    .eq('category', 'racial-profiling')
    .order('created_at', { ascending: false })

  const items = (data ?? []).map((p) => ({
    id:                  p.id,
    href:                `/research/racial-profiling/${p.id}`,
    title:               p.title,
    author:              p.author ?? '',
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-10">
      ...
      {items.length === 0 ? (
        ...
        <RacialProfilingClient items={items} />
      )}
    </div>
  )
}
